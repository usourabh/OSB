...
{
    public partial class EarningPerBusPerDay : Form
    {
        int OsbId = 0;
        int Year = 0;
        int Month = 0;
        ...
        public EarningPerBusPerDay(int OsbId, int Year, int Month, string finYear, string MonthName)
        {
            InitializeComponent();
            this.OsbId = OsbId;
            this.Year = Year;
        ...
        {
            {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    try
                    {
                        ...
                        {
                            SqlCommand cmd = new SqlCommand("INSERT INTO tbl_BarEarningPerBusPerDay ([OsbId],[Month],[Value]) VALUES (@OsbId,@Month,@Value)", con);
                            cmd.Parameters.AddWithValue("@OsbId", OsbId);
        ...
        private void EarningPerBusPerDay_Load(object sender, EventArgs e)
        {
            dataGridView1.DataSource = BindEarningPerBusPerDay();
            BindIndexPage(OsbId);
        }
