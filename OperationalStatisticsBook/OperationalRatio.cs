...
namespace OperationalStatisticsBook
{
    public partial class OperationalRatio : Form
    {
        ...
        public OperationalRatio(int OsbId, int Year, int Month, string finYear, string MonthName)
        {
            InitializeComponent();
            this.OsbId = OsbId;
            this.Year = Year;
        ...
        {
            DeleteExisitingTableRecord("tbl_BarOperationalRatio", OsbId);
            dataGridView1.DataSource = BindOperationalRatio();
        ...
        {
            {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    try
                    {
                        ...
                        {
                            SqlCommand cmd = new SqlCommand("INSERT INTO tbl_BarOperationalRatio ([OsbId],[Month],[Value]) VALUES (@OsbId,@Month,@Value)", con);
                            cmd.Parameters.AddWithValue("@OsbId", OsbId);
