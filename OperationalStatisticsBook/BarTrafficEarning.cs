...
{
    public partial class BarTrafficEarning : Form
    {
        ...
        public BarTrafficEarning(int OsbId, int Year, int Month, string finYear, string MonthName)
        {
            InitializeComponent();
            this.OsbId = OsbId;
            this.Year = Year;
        ...
        {
            {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    try
                    {
                        ...
                        {
                            SqlCommand cmd = new SqlCommand("INSERT INTO tbl_BarTrafficEarning  ([OsbId],[Month],[Value]) VALUES (@OsbId,@Month,@Value)", con);
                            cmd.Parameters.AddWithValue("@OsbId", OsbId);
