...
{
    public partial class rptCategorywiseStaffPositionAsOn : Form
    {
        int OsbId = 0;
        int Year = 0;
        int Month = 0;
        ...
        public rptCategorywiseStaffPositionAsOn(int OsbId, int Year, int Month, string finYear, string MonthName)
        {
            InitializeComponent();
            this.OsbId = OsbId;
            this.Year = Year;
        ...
        private void rptCategorywiseStaffPositionAsOn_Load(object sender, EventArgs e)
        {
            var MonthList = GlobalMaster.GetPrevousMonthList(Month, Year, 2);
            DataTable dtReportData = GetData();
            ReportDataSource datasource = new ReportDataSource("rptCategorywiseStaffPositionAsOn", dtReportData);
